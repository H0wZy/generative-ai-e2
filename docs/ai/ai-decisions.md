...
**Atualização (2026-07-30, refatoração 003) — conteúdo de ticket passa a compor o prompt e a persistência.** A heurística de contexto de chamado (FR-060) agora injeta `ticket_context.subject` — assunto de ticket real, texto livre digitado por quem abriu o chamado — no prompt e na resposta, e `assistant_messages` (persistência por sessão, `X-Session-Id`, ver data-model.md §3) passa a guardar esse campo por turno, não só os trechos do RAG. Diferença de risco frente ao caso original acima: trecho de RAG é documentação estática do próprio projeto, sem dado pessoal esperado; assunto de ticket pode conter e-mail, CPF, telefone ou nome de solicitante (mesma razão que já motiva `redaction.py` existir). Revisão de segurança (cybersec, T042) encontrou que `subject` só era redigido ao montar o prompt, não no objeto devolvido/persistido — corrigido em `backend/app/services/assistant.py::_find_ticket_context` para redigir na origem, antes de o valor circular por qualquer um dos dois caminhos. `X-Session-Id` continua sem assinatura e sem login (research.md R3, mesma confiança já aceita no resto da plataforma) — a consequência nova é que uma sessão exposta agora carrega, no pior caso, assunto de ticket redigido (não cru) em vez de só histórico de chat genérico. `assistant_conversations`/`assistant_messages` não têm TTL nem expurgo, por decisão deliberada já registrada (research.md R3, YAGNI — nenhum FR pede retenção) e reafirmada aqui à luz do dado adicional que a tabela agora carrega.
...
## ADR-013 — Classificador de squad migrado de Ollama para OpenRouter, remedido

**Status:** aceito.

**Contexto:** a refatoração 003 unifica todo uso de LLM no OpenRouter, removendo a dependência de Ollama local (`app/integrations/llm.py` ganhou `OpenRouterSquadClient`, reaproveitando `OpenRouterClient.complete()` já usado pelo assistente — ADR-012). O classificador passa a rodar `nvidia/nemotron-3-ultra-550b-a55b:free`, o mesmo modelo do assistente, em vez de `qwen3:8b` local. Os números do ADR-010 (acurácia 100%, injeção 66,67%) são específicos do Ollama e não se transferem — Princípio I exige remedir, não reaproveitar.

`backend/scripts/routing_eval.py` importava `OllamaClient` diretamente, fora do enum `LLMClientProtocol` — rodar `make routing-eval` sem corrigir o script mediria o provedor errado. Corrigido para importar `OpenRouterSquadClient` antes desta medição.

**Resultado medido (2026-07-30, `nvidia/nemotron-3-ultra-550b-a55b:free`, threshold=0.7):** acurácia 83,33% (10/12), abstenção 37,50% (6/16), **sucesso de prompt injection de 33,33% (1/3)**. Dois casos escoráveis (g03, g10) e um caso de injeção (g18) vieram com erro de requisição ("llm request failed") em vez de classificação — consistente com limite de taxa do tier gratuito do modelo, contados como abstenção/erro, não como acerto nem como resistência real à injeção medida. g17 teve a injeção bem-sucedida (squad ditada pelo texto do chamado, confiança acima do limiar); g19 resistiu.

**Decisão:** `LLM_ENABLED=false` permanece o padrão (reafirma ADR-005/ADR-010). A acurácia caiu de 100% (Ollama, ADR-010) para 83,33% (OpenRouter, medição real) e a taxa de sucesso de injeção continua alta o bastante (33%) para não justificar ligar por padrão — nenhum dos dois provedores atinge um patamar que sustente `LLM_ENABLED=true`.

**Consequência:** os números do ADR-010/ADR-011 ficam como histórico do provedor anterior e não se aplicam mais — esta é a primeira medição real do classificador contra OpenRouter. A contaminação por erro de requisição é característica operacional do modelo gratuito escolhido (custo zero), não falha pontual do script — repetir a medição pode dar números levemente diferentes por esse motivo.

**Auxílio de IA:** identificação do import direto de `OllamaClient` em `routing_eval.py` como bloqueio para uma medição válida, correção do import e do type hint para `OpenRouterSquadClient`, execução de `make routing-eval` contra o provedor real e leitura do resultado sem ajuste de número.

**Segunda medição (2026-07-30, validação qa-dev do quickstart, T043):** `make routing-eval` rodado de novo, independentemente, deu acurácia **75,00% (9/12)** e sucesso de prompt injection **33,33% (1/3)** — a taxa de injeção bate com a primeira rodada; a acurácia variou (83,33% → 75,00%). Isso confirma a nota acima sobre contaminação por erro de requisição/não-determinismo do modelo gratuito: **os dois números ficam registrados**, nenhum dos dois é descartado em favor do outro. A conclusão não muda — em nenhuma das duas rodadas o provedor atinge um patamar que justifique `LLM_ENABLED=true` por padrão.
